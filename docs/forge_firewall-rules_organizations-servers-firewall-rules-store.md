...
```
  forge firewall-rules organizations-servers-firewall-rules-store --organization <value> --server-param 383984 --name <value> --type allow
```
...
* `forge firewall-rules organizations-servers-firewall-rules-store --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge firewall-rules organizations-servers-firewall-rules-store --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge firewall-rules organizations-servers-firewall-rules-store --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
