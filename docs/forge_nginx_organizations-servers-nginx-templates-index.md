...
```
  forge nginx organizations-servers-nginx-templates-index --organization <value> --server-param 429029
```
...
* `forge nginx organizations-servers-nginx-templates-index --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge nginx organizations-servers-nginx-templates-index --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
