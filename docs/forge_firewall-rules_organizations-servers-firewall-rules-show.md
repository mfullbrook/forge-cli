...
```
  forge firewall-rules organizations-servers-firewall-rules-show --organization <value> --server-param 626909 --rule 462429
```
...
* `forge firewall-rules organizations-servers-firewall-rules-show --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge firewall-rules organizations-servers-firewall-rules-show --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
