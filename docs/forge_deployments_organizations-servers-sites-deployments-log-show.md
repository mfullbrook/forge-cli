...
```
  forge deployments organizations-servers-sites-deployments-log-show --organization <value> --server-param 634865 --site 824101 --deployment 929929
```
...
* `forge deployments organizations-servers-sites-deployments-log-show --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge deployments organizations-servers-sites-deployments-log-show --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
