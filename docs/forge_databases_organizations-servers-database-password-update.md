...
```
  forge databases organizations-servers-database-password-update --organization <value> --server-param 171507 --password cDfIMQoihIBizAj
```
...
* `forge databases organizations-servers-database-password-update --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge databases organizations-servers-database-password-update --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge databases organizations-servers-database-password-update --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
