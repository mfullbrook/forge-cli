...
* `forge storage-providers organizations-storage-providers-update --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge storage-providers organizations-storage-providers-update --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge storage-providers organizations-storage-providers-update --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
