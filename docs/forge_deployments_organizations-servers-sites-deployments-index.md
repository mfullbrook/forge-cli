...
```
  forge deployments organizations-servers-sites-deployments-index --organization <value> --server-param 295574 --site 952686
```
...
* `forge deployments organizations-servers-sites-deployments-index --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge deployments organizations-servers-sites-deployments-index --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
