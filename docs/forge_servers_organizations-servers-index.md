...
```
forge servers organizations-servers-index [organization] [flags]
```
...
* `forge servers organizations-servers-index --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge servers organizations-servers-index --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
