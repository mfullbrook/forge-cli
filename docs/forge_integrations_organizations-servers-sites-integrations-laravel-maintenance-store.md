...
```
  forge integrations organizations-servers-sites-integrations-laravel-maintenance-store --organization <value> --server-param 696276 --site 480485 --status 304
```
...
* `forge integrations organizations-servers-sites-integrations-laravel-maintenance-store --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge integrations organizations-servers-sites-integrations-laravel-maintenance-store --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge integrations organizations-servers-sites-integrations-laravel-maintenance-store --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
