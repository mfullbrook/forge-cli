...
```
  forge servers organizations-servers-php-versions-configs-cli-update --organization <value> --server-param 121987 --php-version 680535 --config-param <value>
```
...
* `forge servers organizations-servers-php-versions-configs-cli-update --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge servers organizations-servers-php-versions-configs-cli-update --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge servers organizations-servers-php-versions-configs-cli-update --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
