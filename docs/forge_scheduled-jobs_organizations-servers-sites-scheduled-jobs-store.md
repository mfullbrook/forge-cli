...
```
  forge scheduled-jobs organizations-servers-sites-scheduled-jobs-store --organization <value> --server-param 181290 --site 944759 --command echo $(whoami) --user root
```
...
* `forge scheduled-jobs organizations-servers-sites-scheduled-jobs-store --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge scheduled-jobs organizations-servers-sites-scheduled-jobs-store --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge scheduled-jobs organizations-servers-sites-scheduled-jobs-store --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
