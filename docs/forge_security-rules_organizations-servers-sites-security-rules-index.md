...
```
  forge security-rules organizations-servers-sites-security-rules-index --organization <value> --server-param 863470 --site 589556
```
...
* `forge security-rules organizations-servers-sites-security-rules-index --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge security-rules organizations-servers-sites-security-rules-index --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
