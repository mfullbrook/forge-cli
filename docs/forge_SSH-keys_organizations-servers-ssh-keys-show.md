...
```
  forge SSH-keys organizations-servers-ssh-keys-show --organization <value> --server-param 383622 --key 444641
```
...
* `forge SSH-keys organizations-servers-ssh-keys-show --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge SSH-keys organizations-servers-ssh-keys-show --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
