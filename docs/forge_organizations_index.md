...
```
forge organizations index [flags]
```
...
* `forge organizations index --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge organizations index --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
