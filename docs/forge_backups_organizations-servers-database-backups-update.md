## forge backups organizations-servers-database-backups-update
...
```
forge backups organizations-servers-database-backups-update [flags]
```

### Examples

```
  forge backups organizations-servers-database-backups-update --organization <value> --server-param 450816 --backup-configuration 120221 --storage-provider-id 492562 --frequency hourly
```
...
```
  ...
  -h, --help                        help for organizations-servers-database-backups-update
...
* `forge backups organizations-servers-database-backups-update --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge backups organizations-servers-database-backups-update --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge backups organizations-servers-database-backups-update --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
