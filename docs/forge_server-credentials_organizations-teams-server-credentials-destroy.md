...
* `forge server-credentials organizations-teams-server-credentials-destroy --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge server-credentials organizations-teams-server-credentials-destroy --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
