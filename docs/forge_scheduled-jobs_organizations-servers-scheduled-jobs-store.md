## forge scheduled-jobs organizations-servers-scheduled-jobs-store
...
```
forge scheduled-jobs organizations-servers-scheduled-jobs-store [flags]
```

### Examples

```
  forge scheduled-jobs organizations-servers-scheduled-jobs-store --organization <value> --server-param 459969 --command echo $(whoami) --user root --frequency nightly
```
...
```
  ...
  -h, --help                  help for organizations-servers-scheduled-jobs-store
...
* `forge scheduled-jobs organizations-servers-scheduled-jobs-store --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge scheduled-jobs organizations-servers-scheduled-jobs-store --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge scheduled-jobs organizations-servers-scheduled-jobs-store --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
