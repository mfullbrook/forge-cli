...
```
  forge monitors organizations-servers-monitors-store --organization <value> --server-param 555618 --type free_memory --operator gte --threshold 90
```
...
* `forge monitors organizations-servers-monitors-store --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge monitors organizations-servers-monitors-store --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge monitors organizations-servers-monitors-store --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
