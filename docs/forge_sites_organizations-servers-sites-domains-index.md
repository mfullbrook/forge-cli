...
```
  forge sites organizations-servers-sites-domains-index --organization <value> --server-param 528382 --site 439382
```
...
* `forge sites organizations-servers-sites-domains-index --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge sites organizations-servers-sites-domains-index --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
