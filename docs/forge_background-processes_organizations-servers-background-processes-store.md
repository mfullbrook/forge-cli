...
```
  forge background-processes organizations-servers-background-processes-store --organization <value> --server-param 627041 --name Custom command runner --command php artisan custom:command --user forge
```
...
* `forge background-processes organizations-servers-background-processes-store --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge background-processes organizations-servers-background-processes-store --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge background-processes organizations-servers-background-processes-store --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
