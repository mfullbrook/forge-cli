...
```
  forge sites organizations-servers-sites-heartbeats-update --organization <value> --server-param 12665 --site 354926 --heartbeat 178389 --name My Heartbeat
```
...
* `forge sites organizations-servers-sites-heartbeats-update --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge sites organizations-servers-sites-heartbeats-update --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge sites organizations-servers-sites-heartbeats-update --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
