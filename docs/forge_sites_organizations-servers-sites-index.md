...
```
  forge sites organizations-servers-sites-index --organization <value> --server-param 235028
```
...
* `forge sites organizations-servers-sites-index --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge sites organizations-servers-sites-index --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
