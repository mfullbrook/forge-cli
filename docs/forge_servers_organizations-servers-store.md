...
```
forge servers organizations-servers-store [organization] [flags]
```
...
* `forge servers organizations-servers-store --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge servers organizations-servers-store --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge servers organizations-servers-store --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
