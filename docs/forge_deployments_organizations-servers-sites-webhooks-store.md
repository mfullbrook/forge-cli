...
```
  forge deployments organizations-servers-sites-webhooks-store --organization <value> --server-param 441432 --site 607400 --url https://powerful-outlaw.com
```
...
* `forge deployments organizations-servers-sites-webhooks-store --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge deployments organizations-servers-sites-webhooks-store --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `forge deployments organizations-servers-sites-webhooks-store --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
