## forge sites organizations-sites-index
...
```
forge sites organizations-sites-index [organization] [flags]
```

### Examples

```
  forge sites organizations-sites-index --organization <value>
```
...
```
  ...
  -h, --help                  help for organizations-sites-index
...
* `forge sites organizations-sites-index --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `forge sites organizations-sites-index --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
