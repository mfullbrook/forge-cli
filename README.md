...
```bash
forge background-processes organizations-servers-background-processes-index --http 'Bearer test_token' --organization '<value>' --server-param 170230 --page-size 30 --filter-user forge --filter-site-id 1 --filter-directory /home/forge/forge.laravel.com
...
| Run | You get |
|-----|---------|
| `forge --help`, `forge user me --help` | Commands by category, runnable examples, flags |
| `forge --usage`, `forge user me --usage` | The command surface as machine-readable [KDL](https://kdl.dev): commands, aliases, flags, defaults, env vars, config keys |
| `forge background-processes organizations-servers-background-processes-store --schema` | The exact JSON Schema of the command's request body (all `$ref`s bundled) — build a valid `--body` from it |
| `forge user me --dry-run` | The exact HTTP request (method, URL, headers, body), with no credentials or network call |
| `forge user me --output-format json` (or `--jq`) | Machine-readable output |
...
forge background-processes organizations-servers-background-processes-store --schema
```
...
forge user me --dry-run

# Machine preview: compact JSON on stdout and silent stderr
forge user me --dry-run --output-format json
```
...
```bash
# JSON on stdout
forge user me --output-format json
...
forge user me --interactive

# Open the guided configuration form
forge configure --interactive
...
```bash
forge --http "$FORGE_HTTP" --oauth2 "$FORGE_OAUTH2" user me
```
...
* [`background-processes`](docs/forge_background-processes.md) - Operations for background-processes
...
* [`backups`](docs/forge_backups.md) - Operations for backups
...
* [`commands`](docs/forge_commands.md) - Operations for commands
...
* [`databases`](docs/forge_databases.md) - Operations for databases
...
* [`deployments`](docs/forge_deployments.md) - Operations for deployments
...
* [`firewall-rules`](docs/forge_firewall-rules.md) - Operations for firewall-rules
...
* [`integrations`](docs/forge_integrations.md) - Operations for integrations
...
* [`logs`](docs/forge_logs.md) - Operations for logs
...
* [`monitors`](docs/forge_monitors.md) - Operations for monitors
...
* [`nginx`](docs/forge_nginx.md) - Operations for nginx
...
* [`organizations`](docs/forge_organizations.md) - Operations for organizations
...
* [`providers`](docs/forge_providers.md) - Operations for providers
...
* [`recipes`](docs/forge_recipes.md) - Operations for recipes
...
* [`redirect-rules`](docs/forge_redirect-rules.md) - Operations for redirect-rules
...
* [`roles`](docs/forge_roles.md) - Operations for roles
...
* [`SSH-keys`](docs/forge_SSH-keys.md) - Operations for SSH-keys
...
* [`scheduled-jobs`](docs/forge_scheduled-jobs.md) - Operations for scheduled-jobs
...
* [`security-rules`](docs/forge_security-rules.md) - Operations for security-rules
...
* [`server-credentials`](docs/forge_server-credentials.md) - Operations for server-credentials
...
* [`servers`](docs/forge_servers.md) - Operations for servers
...
* [`sites`](docs/forge_sites.md) - Operations for sites
...
* [`storage-providers`](docs/forge_storage-providers.md) - Operations for storage-providers
...
* [`teams`](docs/forge_teams.md) - Operations for teams
...
* [`user`](docs/forge_user.md) - Operations for user
...
```bash
forge --server-url https://custom-api.example.com user me
```
...
forge user me --output-format toon

# Pipe JSON to jq without using --output-format
forge user me --output-format json | jq '.'
```
...
forge user me --output-format json > output.json 2> error.log
if [ $? -ne 0 ]; then
