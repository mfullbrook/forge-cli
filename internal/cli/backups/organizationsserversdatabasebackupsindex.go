...
var organizationsServersDatabaseBackupsIndexCmdMeta = []flagutil.FlagMeta{
	...
	{FlagName: "filter-name", FieldPath: "FilterName", Kind: flagutil.FlagKindString, Optional: true, Description: "The name of the backup configuration."},
	{FlagName: "filter-status", FieldPath: "FilterStatus", Kind: flagutil.FlagKindString, Optional: true, Description: "The status of the backup configuration."},
}

// initOrganizationsServersDatabaseBackupsIndexCmd initializes the organizations-servers-database-backups-index command.
func initOrganizationsServersDatabaseBackupsIndexCmd(parent *cobra.Command) error {
	...
	if err := flagutil.ValidateMeta[operations.OrganizationsServersDatabaseBackupsIndexRequest](organizationsServersDatabaseBackupsIndexCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for organizations-servers-database-backups-index: %w", err)
	}
	parent.AddCommand(cmd)
	return nil
}

// runOrganizationsServersDatabaseBackupsIndexCmd executes the organizations-servers-database-backups-index command.
func runOrganizationsServersDatabaseBackupsIndexCmd(cmd *cobra.Command, args []string) error {
	...
	res, err := s.Backups.OrganizationsServersDatabaseBackupsIndex(cmd.Context(), *req, sdkOpts...)
	if err != nil {
