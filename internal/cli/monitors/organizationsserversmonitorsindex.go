...
var organizationsServersMonitorsIndexCmdMeta = []flagutil.FlagMeta{
	...
	{FlagName: "filter-status", FieldPath: "FilterStatus", Kind: flagutil.FlagKindString, Optional: true, Description: "The status of the monitor."},
	{FlagName: "filter-state", FieldPath: "FilterState", Kind: flagutil.FlagKindString, Optional: true, Description: "The state of the monitor."},
	{FlagName: "filter-type", FieldPath: "FilterType", Kind: flagutil.FlagKindString, Optional: true, Description: "The type of the monitor."},
	{FlagName: "filter-notify", FieldPath: "FilterNotify", Kind: flagutil.FlagKindString, Optional: true, Description: "The email address to notify when the monitor is in an alert state."},
}

// initOrganizationsServersMonitorsIndexCmd initializes the organizations-servers-monitors-index command.
func initOrganizationsServersMonitorsIndexCmd(parent *cobra.Command) error {
	...
	if err := flagutil.ValidateMeta[operations.OrganizationsServersMonitorsIndexRequest](organizationsServersMonitorsIndexCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for organizations-servers-monitors-index: %w", err)
	}
	parent.AddCommand(cmd)
	return nil
}

// runOrganizationsServersMonitorsIndexCmd executes the organizations-servers-monitors-index command.
func runOrganizationsServersMonitorsIndexCmd(cmd *cobra.Command, args []string) error {
	...
	res, err := s.Monitors.OrganizationsServersMonitorsIndex(cmd.Context(), *req, sdkOpts...)
	if err != nil {
