...
var organizationsServersEventsIndexCmdMeta = []flagutil.FlagMeta{
	...
	{FlagName: "filter-initiated-by", FieldPath: "FilterInitiatedBy", Kind: flagutil.FlagKindString, Optional: true, Description: "The user ID of the event initiator."},
	{FlagName: "filter-ran-as", FieldPath: "FilterRanAs", Kind: flagutil.FlagKindString, Optional: true, Description: "The server user that the event was run as."},
}

// initOrganizationsServersEventsIndexCmd initializes the organizations-servers-events-index command.
func initOrganizationsServersEventsIndexCmd(parent *cobra.Command) error {
	...
	if err := flagutil.ValidateMeta[operations.OrganizationsServersEventsIndexRequest](organizationsServersEventsIndexCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for organizations-servers-events-index: %w", err)
	}
	parent.AddCommand(cmd)
	return nil
}

// runOrganizationsServersEventsIndexCmd executes the organizations-servers-events-index command.
func runOrganizationsServersEventsIndexCmd(cmd *cobra.Command, args []string) error {
	...
	res, err := s.Servers.OrganizationsServersEventsIndex(cmd.Context(), *req, sdkOpts...)
	if err != nil {
